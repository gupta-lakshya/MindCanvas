        ...
        <div className="rounded-2xl rounded-bl-md bg-white border border-sand/60 p-4 shadow-sm">
          ...
          <div >
            {/* {[1, 2, 3].map((panel) => ( */}
              ...
              >
                {/* Put generated images in `frontend/public/generated_image/` and reference them from the app as `/generated_image/output_image.jpg` */}
                <img
                  src={`./${text.replace(/"/g, '')}.jpg`}
                  alt="Comic panel placeholder"
...
export default function ChatPage() {
  ...
  const handleSubmit = async() => {
    ...
    try {
      await fetch("http://localhost:8000/ai/generate", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
